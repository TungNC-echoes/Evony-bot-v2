...
def refresh_market(device_id=None):
    """Refresh chợ đen. Lần đầu trong chợ dùng refresh_basic nếu chưa có refresh."""
    try:
        ...
        if find_and_click_button("market/refresh", device_id=device_id, wait_time=0.5):
            ...
            time.sleep(2)
            return True
        if find_and_click_button("market/refresh_basic", device_id=device_id, wait_time=0.5):
            print("Đã refresh chợ đen bằng refresh_basic")
            time.sleep(2)
            return True
