...
def attack_boss(boss_name, x_coord, y_coord, image_folder, troops_count, start_time=None, threshold=0.8, boss_info=None, use_assistant_general=False):
            ...
            for image_file in os.listdir(image_folder):
                if image_file.endswith(('.jpg', '.JPG', '.png', '.PNG')):
                    ...
                    if boss_info and find_and_click_button(f"attack/{boss_info['folder']}/{image_name}", 'none', 1, 2, threshold):
                        ...
                        result = execute_attack_sequence(start_time, troops_count, use_assistant_general)
...
def get_boss_config(boss_name, troops_count=500000):
    ...
    specific_boss_types = {
        ...
        "Vệ Binh Đền Thường": {
            "folder": "ve_binh",
            "image_path": get_image_path("buttons/attack/ve_binh/"),
            "troops_count": str(troops_count),
            "threshold": 0.7
        },
        "Nhân Sư": {
        ...
        "Cướp Biển Barbary Thường": {
            "folder": "barbary",
            "image_path": get_image_path("buttons/attack/barbary/"),
            "troops_count": str(troops_count),
            "threshold": 0.75
        },
    }
...
def attack_selected_bosses(selected_groups, bosses, start_time=None, troops_count=500000, use_assistant_general=False):
...
def execute_attack_sequence(start_time=None, troops_count="300000", use_assistant_general=False):
        ...
        while retry_count < max_retries:
            ...
            if check_button_exists("doi_quan_san_co", 'none', 0.85):
                if find_and_click_button("doi_quan_san_co"):
                    ...
                    # Click lần lượt các nút còn lại (tướng chính)
                    ...
                    # Optional: chọn tướng phụ trước khi nhập quân
                    if use_assistant_general:
                        print("Đang chọn tướng phụ...")
                        if find_and_click_button("chon_tuong_phu", 'none', 1, 1, 0.8):
                            time.sleep(2)
                            if find_and_click_button("chon", 'none', 1, 1, 0.8):
                                print("Đã chọn tướng phụ")
                                time.sleep(2)
                            else:
                                print("Không thể click Select tướng phụ, tiếp tục nhập quân")
                        else:
                            print("Không thấy nút chon_tuong_phu, bỏ qua tướng phụ")

                    if not find_and_click_button("attack/nhap_quan", 'none', 1, 1, 0.8):
                        print("Không thể click vào nút attack/nhap_quan")
                        return False
                    time.sleep(2)
