...
class UIBuilder:
    ...
    def setup_ui(self):
        ...
        feature_dropdown = ttk.Combobox(row2_frame, textvariable=self.gui.feature_var, 
                                       values=["⚔️ Rally", "🛒 Buy Meat", "🎯 War", "👹 Attack Boss", "📦 Open Items", "🛒 Buy General", "⚔️ Advanced Rally", "🎯 Advanced War"],
        ...
        # iScout account settings (boss locations)
        self.setup_iscout_settings_ui(left_panel)
        ...
        scrollable_features_frame.rowconfigure(3, weight=1)     # Row thứ 4 (Advanced Rally / War + Boss)
        ...
        # Advanced Rally + Advanced War + Select Boss gom một nhóm
        advanced_group = ttk.LabelFrame(
            scrollable_features_frame,
            text="⚔️ Advanced Rally / War",
            padding="10",
            style="Panel.TLabelframe",
        )
        advanced_group.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), padx=5, pady=5)
        advanced_group.columnconfigure(0, weight=1, minsize=670)
        advanced_group.columnconfigure(1, weight=1, minsize=670)
        advanced_group.rowconfigure(0, weight=1)

        self.create_feature_container(advanced_group, "⚔️ Advanced Rally", "advanced_rally", 0, 0)
        self.create_feature_container(advanced_group, "🎯 Advanced War (No General)", "advanced_war", 0, 1)
        self.setup_boss_selection_ui(advanced_group)
    ...
    def create_feature_container(self, parent, title, feature_key, row, col, columnspan=1):
        ...
        if feature_key == "attack_boss":
            ...
            troops_row = ttk.Frame(input_frame)
            troops_row.pack(side=tk.TOP, fill=tk.X)
            
            # Troops count input
            ttk.Label(troops_row, text="Troops:", style="Count.TLabel").pack(side=tk.LEFT, padx=(0, 5))
            
            troops_var = tk.StringVar()
            troops_entry = ttk.Entry(troops_row, textvariable=troops_var, width=10, font=("Arial", 9))
            ...
            # Validation label
            validation_label = ttk.Label(troops_row, text="⚠️ Nhập số quân", style="Warning.TLabel")
            ...
            assistant_var = tk.BooleanVar(value=False)
            assistant_check = ttk.Checkbutton(
                input_frame,
                text="Chọn tướng phụ",
                variable=assistant_var
            )
            assistant_check.pack(side=tk.TOP, anchor=tk.W, pady=(4, 0))
            setattr(self.gui, f"{feature_key}_select_assistant_var", assistant_var)

        if feature_key == "open_items":
            options_frame = ttk.Frame(control_frame)
            options_frame.pack(side=tk.LEFT, padx=(0, 10))

            resource_var = tk.BooleanVar(value=True)
            gems_var = tk.BooleanVar(value=False)
            ttk.Checkbutton(options_frame, text="Mở resource", variable=resource_var).pack(side=tk.LEFT, padx=(0, 8))
            ttk.Checkbutton(options_frame, text="Mở ngọc", variable=gems_var).pack(side=tk.LEFT)
            setattr(self.gui, f"{feature_key}_open_resource_var", resource_var)
            setattr(self.gui, f"{feature_key}_open_gems_var", gems_var)
        ...
        if feature_key in ("rally", "advanced_rally"):
            ...
            assistant_check.pack(side=tk.LEFT, padx=(0, 10))
            setattr(self.gui, f"{feature_key}_select_assistant_var", assistant_var)
    ...
    BASIC_BOSS_EXCLUDED = {"viking", "cerberus", "bayar"}

    def _is_basic_boss(self, boss_name):
        return str(boss_name).strip().lower() not in self.BASIC_BOSS_EXCLUDED

    def setup_boss_selection_ui(self, parent):
        """Tạo UI chọn boss cho Advanced Rally / War"""
        boss_frame = ttk.LabelFrame(parent, text="👹 Select Boss Types", padding="10", style="Panel.TLabelframe")
        boss_frame.grid(row=1, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), padx=5, pady=(8, 0))
        ...
        basic_boss_btn = ttk.Button(lang_frame, text="Basic Boss",
                                    command=self.check_basic_bosses, width=12)
        basic_boss_btn.pack(side=tk.LEFT, padx=(10, 5))

        all_boss_btn = ttk.Button(lang_frame, text="All Boss",
                                  command=self.check_all_bosses, width=10)
        all_boss_btn.pack(side=tk.LEFT, padx=(0, 5))
    ...
    def setup_boss_checkboxes(self):
        ...
        for i, (boss_name, filename) in enumerate(boss_files):
            ...
            var = tk.BooleanVar(value=self._is_basic_boss(boss_name))
    ...
    def check_basic_bosses(self):
        """Check Basic Boss: tất cả trừ viking, cerberus, bayar."""
        try:
            if hasattr(self.gui, "boss_vars"):
                for boss_name, var in self.gui.boss_vars.items():
                    var.set(self._is_basic_boss(boss_name))
        except Exception as e:
            print(f"❌ Lỗi khi check basic bosses: {e}")
    ...
    def setup_iscout_settings_ui(self, parent):
        """iScout account + mode settings for boss location fetch."""
        from utils.iscout_config import load_iscout_config

        frame = ttk.LabelFrame(parent, text="🧭 iScout (Boss Locations)", padding="8", style="Panel.TLabelframe")
        frame.grid(row=2, column=0, sticky=(tk.W, tk.E), pady=(8, 0))
        frame.columnconfigure(1, weight=1)

        cfg = load_iscout_config()

        ttk.Label(frame, text="Email:", style="Count.TLabel").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.gui.iscout_email_var = tk.StringVar(value=cfg.get("email", ""))
        ttk.Entry(frame, textvariable=self.gui.iscout_email_var, width=22).grid(
            row=0, column=1, sticky=(tk.W, tk.E), pady=2, padx=(4, 0)
        )

        ttk.Label(frame, text="Password:", style="Count.TLabel").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.gui.iscout_password_var = tk.StringVar(value=cfg.get("password", ""))
        ttk.Entry(frame, textvariable=self.gui.iscout_password_var, show="*", width=22).grid(
            row=1, column=1, sticky=(tk.W, tk.E), pady=2, padx=(4, 0)
        )

        ttk.Label(frame, text="Mode:", style="Count.TLabel").grid(row=2, column=0, sticky=tk.W, pady=2)
        mode_value = cfg.get("mode", "managed")
        mode_label = "Managed (tự mở Chrome)" if mode_value == "managed" else "CDP (Chrome đang mở)"
        self.gui.iscout_mode_var = tk.StringVar(value=mode_label)
        mode_combo = ttk.Combobox(
            frame,
            textvariable=self.gui.iscout_mode_var,
            values=["Managed (tự mở Chrome)", "CDP (Chrome đang mở)"],
            state="readonly",
            width=20,
        )
        mode_combo.grid(row=2, column=1, sticky=(tk.W, tk.E), pady=2, padx=(4, 0))

        ttk.Label(frame, text="CDP port:", style="Count.TLabel").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.gui.iscout_port_var = tk.StringVar(value=str(cfg.get("cdp_port", 9014)))
        ttk.Entry(frame, textvariable=self.gui.iscout_port_var, width=10).grid(
            row=3, column=1, sticky=tk.W, pady=2, padx=(4, 0)
        )

        btn_row = ttk.Frame(frame)
        btn_row.grid(row=4, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(6, 0))

        ttk.Button(
            btn_row, text="💾 Lưu", width=8, command=self.gui.save_iscout_settings, style="Action.TButton"
        ).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(
            btn_row, text="🔑 Login", width=8, command=self.gui.test_iscout_login, style="Action.TButton"
        ).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(
            btn_row, text="📡 Lấy Boss", width=10, command=self.gui.fetch_iscout_bosses, style="Action.TButton"
        ).pack(side=tk.LEFT)
